...
DB は Render Postgres ではなく Neon を使う前提にしています。
そのため、Render 側では DB リソースを作らず、`DATABASE_URL` を手動設定します。
...
production は `DATABASE_URL` をそのまま使うようにしました。
あわせて、`primary` / `cache` / `queue` / `cable` という接続名は
いったん同じ DB を向く alias にしています。

理由:

- Neon / Render どちらを使う場合でも DB 接続 URL を環境変数で渡すのが基本
...
1. `life-simulator-back-prod`
   本番用 Rails API Web Service

2. `life-simulator-front-prod`
...
1. `life-simulator-back-stg`
   ステージング用 Rails API Web Service

2. `life-simulator-front-stg`
...
- `DATABASE_URL`
  Neon 側で作成した接続文字列を Render の環境変数として手動設定する
...
- `DATABASE_URL`
  Neon の接続文字列を Render Dashboard で手動入力
...
Neon では 1 つの project に対して root branch `main` があり、
そこから child branch を作れます。branch ごとに接続先を分けられるため、
本番とステージングの分離に使えます。

今回のおすすめ構成:

- Neon project: `life-simulator`
- Neon branch: `main`
  本番用
- Neon branch: `development`
  ステージング用

Rails 側はどちらも `DATABASE_URL` を使うだけなので、
Render では環境ごとに別の接続文字列を設定します。

本番 back に設定する値:

```text
<Neon main branch の connection string>
```

ステージング back に設定する値:

```text
<Neon development branch の connection string>
```
